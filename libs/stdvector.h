...
template <typename V>
void RegisterVector(const std::string T_AS,  //Template parameter typename in AS - must already be
			        asIScriptEngine* engine) //registered (or be primitive type)!!
{
	typedef vector<V> VectorType;
    typedef vectorRegisterHelper<VectorType, V> FuncBase;
	
	const std::string V_AS = string("vector_") + T_AS;

    assert(engine && "Passed NULL engine pointer to registerVector");

	int error_code = 0;
	error_code = engine->RegisterObjectType(V_AS.c_str(), sizeof(VectorType), asOBJ_VALUE | asOBJ_APP_CLASS_CDA);
